...
- **「没有这个脚本」的三种含义**（2026-09-25 起）：pnpm 报 `Missing script: <名>` 时，vdsh 会把它与
  本次要跑的脚本（`build`，`--clean` 时还有 `clean`）以及 vdsh 自己拼进去的子命令（`run`）对照：
  **报缺的是那个子命令**（例如要跑 `build`，却说没有 `run` 脚本）→ 判**命令拼装错误**（启动器缺陷，
  与仓库无关，清缓存建议此时与现场相反，不再打印）；**报缺的是请求的脚本** → 提示核对仓库
  `package.json` 的 `scripts`（可能是 DSH 改了脚本名）；**报缺的是别的名字**（如构建脚本内部对子包的
  调用）→ 同样指向 `package.json`，但不会归咎 vdsh。
