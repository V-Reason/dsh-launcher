...
- **输出（唯一标准，2026-09-10 起）**：终端输出只有四种东西——转轮 `⠋ 任务性质 47s`（`spinner` 自动追加秒数；消息只写任务名，
  不带 `…`、不写内部计数）、进度 `→ 事实`（`console.progress()` 或 `quiet` 回调返回的归一化进度行）、节点 `vdsh · …`（`console.step()`）、
  结论 `vdsh ✓/✗/⚠`（`console.ok()/die()/fail()/warn()`）；成功任务的**耗时单独一行** `vdsh · 用时 1m46s`。
