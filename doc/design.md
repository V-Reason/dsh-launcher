...
| pnpm 命令**原样**拼接 + 脚本名指纹诊断（2026-09-25） | `_pnpm_command` 曾在路径字符串形式隐式补 `run`，与调用方传的原样 argv 叠加成 `pnpm run run build` → 0.1.7-rc.2 的跨版本构建全灭；离线复测只覆盖 argv 列表形式（不补 `run`），故 30+ 断言全绿而真机全红。改为两种形式同构、`args` 原样（肉眼可核对，杜绝隐式变换），`cleaned` 同时改回「已尝试」语义。诊断加 missing-script 指纹：报缺的恰是 vdsh 拼进去的子命令（`run`）→ 判启动器拼装错误并**撤掉**清缓存建议（给两条互斥建议等于没建议，且判据要窄——否则子包脚本缺失会被诬告成启动器的问题）；报缺的是请求的脚本或别名字 → 指向仓库 `package.json` 的 scripts |
...
- 构建失败的 missing-script 判定是**文本指纹**（`Missing script: X` / `Command "X" not found.`）：pnpm 换措辞即失效，
  失效时只退回原来的通用诊断（不产生错误结论）；**不会**去硬校验仓库 `package.json` 的脚本名——DSH 正常改名不该让启动器硬失败。
